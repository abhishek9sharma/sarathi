...
def check_config_nudge(skip_banner=False):
    ...
    if not global_config_path.exists() and not local_config_path.exists():
        if not skip_banner:
            print_banner()
        print(
            "\033[93mTip: No configuration found (~/.sarathi/config.yaml or sarathi.yaml). Run 'sarathi config init' to get started.\033[0m"
        )
        print("-" * 50)
    ...
    try:
        ...
        if not parsed_args.op:
            ...
            if not check_config_nudge(skip_banner=True):
                ...
                print(
                    "\033[94mTip: Sarathi is configured and ready. Try 'sarathi chat' or 'sarathi git autocommit'.\033[0m"
                )
                print("-" * 50)
