...
def setup_args(subparsers, opname):
    ...
    gentest_parser = code_sub_cmd.add_parser(
        "gentest", help="Generate unit tests for a Python file"
    )
    ...
    edit_parser = code_sub_cmd.add_parser(
        "edit", help="Edit code based on natural language request"
    )
    edit_parser.add_argument(
        "request", help="Natural language description of what to do"
    )
    edit_parser.add_argument(
        "-c", "--context", nargs="+", help="Context files to provide to the agent"
    )
...
def execute_cmd(args):
    ...
    if args.code_sub_cmd == "gentest":
        result = agent.generate_tests(
            source_file=args.file, test_framework=args.framework
        )
        print("\n" + "=" * 60)
        print("RESULT:")
        print("=" * 60)
        ...
        result = agent.edit_code(user_request=args.request, context_files=args.context)
        print("\n" + "=" * 60)
        print("RESULT:")
        print("=" * 60)
        print(result)
