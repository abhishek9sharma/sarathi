...
def resolve_package_info(package_name):
    ...
    try:
        ...
        return {
            ...
            "license": (
                lic
                if lic and lic.lower() not in ["unknown", "n/a", ""]
                else ("MIT" if install_name.lower() == "pytest" else "Unknown")
            ),
        }
    ...
    def build_tree(pkg_name, seen_in_branch):
        ...
        try:
            ...
            for req in sorted(set(clean_reqs)):
                node["dependencies"][req] = build_tree(
                    req, seen_in_branch | {normalized_name}
                )
...
def execute_depgraph(args):
    ...
    if args.package:
        ...
        console.print(
            f"🔍 [bold cyan]Analyzing project code to find root dependencies...[/bold cyan]"
        )
    ...
    console.print(
        f"  • Total unique dependencies: [bold green]{len(seen_globally)}[/bold green]\n"
    )
...
def get_integrity_report(directory="."):
    ...
    for imp in normalized_actual:
        if (
            imp not in clean_declared
            and imp not in stdlib_common
            and imp not in ["sarathi", "setuptools"]
        ):
...
def execute_imports(args):
    ...
    if not args.json:
        console.print(
            f"🔍 Scanning for imports in: [cyan]{os.path.abspath(directory)}[/cyan]"
        )
    ...
    for lib in sorted(lib_to_files.keys()):
        ...
        v_styled = (
            ...
            if v == "stdlib"
                ...
                f"[cyan]{v}[/cyan]"
                if v == "Internal/Unknown"
                else f"[green]{v}[/green]"
            )
        )

        table.add_row(lib, v_styled, info["license"], "\n".join(files))
...
def execute_revdeps(args):
    ...
    console.print(
        f"🔍 [bold cyan]Searching for reverse dependencies of '{args.package}'...[/bold cyan]\n"
    )
    dependents = get_reverse_deps(args.package)

    if not dependents:
        console.print(
            f"[yellow]No installed packages found that depend on '{args.package}'.[/yellow]"
        )
        return

    table = Table(
        title=f"[bold magenta]Dependents of {args.package}[/bold magenta]",
        box=box.SIMPLE,
    )
...
def setup_args(subparsers, opname="sbom"):
    parser = subparsers.add_parser(
        opname, help="Software Bill of Materials (SBOM) tools"
    )
    sbom_subparsers = parser.add_subparsers(dest="sbom_op")

    # Subcommand: imports
    imp_parser = sbom_subparsers.add_parser(
        "imports", help="List external imports and their source locations"
    )
    ...
    dep_parser = sbom_subparsers.add_parser(
        "depgraph", help="Visualize full dependency tree"
    )
    dep_parser.add_argument(
        "path", nargs="?", help="Path to scan for root dependencies"
    )
    dep_parser.add_argument(
        "-p", "--package", help="Graph a specific package instead of the whole project"
    )

    # Subcommand: check
    check_parser = sbom_subparsers.add_parser(
        "check", help="Check for unused or undeclared dependencies"
    )
    check_parser.add_argument(
        "path", nargs="?", help="Path to project (default: current directory)"
    )
    check_parser.add_argument(
        "--fail", action="store_true", help="Exit with non-zero if issues are found"
    )

    # Subcommand: revdeps
    rev_parser = sbom_subparsers.add_parser(
        "revdeps", help="Find which packages depend on a target package"
    )
    rev_parser.add_argument(
        "-p", "--package", required=True, help="Target package name"
    )
