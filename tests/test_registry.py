    ...
    for command, config in CLI_REGISTRY.items():
        if 'args' in config:
            ...
            for arg in config['args']:
                assert 'flags' in arg and 'kwargs' in arg, f"Invalid arg structure in {command}"
                assert isinstance(arg['flags'], list), f"Flags for {command} is not a list"
                assert isinstance(arg['kwargs'], dict), f"Kwargs for {command} is not a dict"
        ...
        if 'args' in config:
            for arg in config['args']:
                if 'type' in arg['kwargs']:
                    ...
                    assert type_str in type_function_map, f"Unknown type {type_str} in {command}"
                    assert callable(type_function_map[type_str]), f"Type {type_str} is not callable in {command}"
...
def test_cli_registry_custom_setup():
    ...
    for command, config in CLI_REGISTRY.items():
        if 'custom_setup' in config:
            assert isinstance(config['custom_setup'], bool), f"Custom setup for {command} is not a boolean"
            assert config['custom_setup'] is True, f"Custom setup for {command} is not True when expected"
